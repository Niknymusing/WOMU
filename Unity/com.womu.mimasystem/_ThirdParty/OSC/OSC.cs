...
public class UDPPacketIO
{
    ...
    public bool Open()
    {
        if (socketsOpen) return true;

        try
        {
            Sender = new UdpClient();
            status = OSCStatus.Connecting;

            //IPEndPoint[] availableEndPoints = System.Net.NetworkInformation.IPGlobalProperties.GetIPGlobalProperties().GetActiveUdpListeners();
            //for (int i = 0; i < availableEndPoints.Length; ++i) Debug.Log(availableEndPoints[i] + ":" + availableEndPoints[i].Port + ", " + availableEndPoints[i].Address);
            //Debug.LogWarning("Has " + localPort + "? " + availableEndPoints.Any(x => x.Port == localPort) + " ::: first available: " + availableEndPoints.First().Port);
            ...
            Receiver.Client.SendTimeout = 1000;
            Receiver.Client.ReceiveTimeout = 1000;
            Receiver.Client.Bind(listenerIp);
            socketsOpen = true;
            return true;
        }
        catch (Exception e)
    ...
    public void SendPacket(byte[] packet, int length)
    {
        if (!IsOpen())
        {
            Debug.Log("Osc wasn't open - attempting to open now");
            Open();
        }

        if (!IsOpen())
        {
            Debug.LogWarning("Osc couldn't be open, aborting.");
            return;
        }
        //Debug.Log("Sending message via " + Sender + " of length: " + length + " to " + remotePort);
    ...
    public Action<OSCStatus> OnStatusChange;
    ...
    public int ReceivePacket(byte[] buffer)
    {
        //if (!IsOpen()) Open();
        if (!IsOpen()) return 0;
        if (ReferenceEquals(Receiver, null)) return 0;
        ...
        try
        {
            ...
            count = Math.Min(buffer.Length, incoming.Length);
...
public class OSC : MonoBehaviour
{
    ...
    public bool ReadyToConnect = true;
    ...
    {
        if (ReaderRunning) Close();
        if (!ReadyToConnect)
        {
            Debug.LogWarning("Still closing connection, not ready to connect yet");
            return;
        }
        ...
        OscPacketIO.OnStatusChange += status => OnStatusChange?.Invoke(status);
        OscPacketIO.Open();
    ...
    {
        // Debug.Log("Setting address handler for key " + key);
    ...
    void Update()
    {
        if(closeOSCPacket) CloseOSCPacketIO();
    ...
    */
    public void Close()
    {
        ...
        if (OscPacketIO != null) OscPacketIO.OnStatusChange = null;
        if (ReaderRunning || closeOSCPacket)
        {
            ReaderRunning = false;
            //ReadThread.Abort();
        }
    ...
    bool closeOSCPacket = false;
    private void CloseOSCPacketIO()
    {
    ...
    private void Read()
    {
        try
        {
            while (ReaderRunning)
            {
                ...
                int length = OscPacketIO == null ? 0 : OscPacketIO.ReceivePacket(buffer);
            ...
            Debug.LogWarning(ReaderRunning + " no longer running");
            closeOSCPacket = true;
        }
        catch (Exception e)
        {
    ...
    public void Send(OscMessage oscMessage)
    {
        if (OscPacketIO == null || !OscPacketIO.IsOpen())
        {
            Debug.Log("Failed to send message as OscPacketIO not open " + OscPacketIO);
            return;
        }
        byte[] packet = new byte[1000];
        int length = OSC.OscMessageToPacket(oscMessage, packet, 1000);
        // THIS CALL ATTEMPTS TO OPEN A CONNECTION IF NOT YET OPEN
